...
(function(window, document) {
  ...
  window.flagstone = function(devElem, devObj) {
    ...
    function Flagstone(dElem, dObj) {
      ...
      function resizeHeight(targetElem) {
        ...
        const elemTop = window.parseInt(elem.style.top,10) + (window.parseInt(elem.style.marginTop,10) || 0); // 65% faster than Regex replace (at least on parseInt)
        ...
        function animateBedHeightFunc() {
          ...
          for (;i<l;i++) {
            ...
            const h = window.parseInt(lastInColumn.style.top,10) + (window.parseInt(lastInColumn.style.marginTop,10) || 0) + lastInColumn.offsetHeight + bedPadding;
        ...
        function animateMarginTops() {
          ...
          const comparisonTop = window.parseInt(elemsToMove[0].style.top,10) - stonesMargin + currMarginTops[0]; // Shouldn't cause reflow like offsetTop would
        ...
        if (watchImages) {
          ...
          for (;i<l;i++) {
            ...
            if (!images[i].naturalWidth) {
              images[i].removeEventListener('load', reset);
              images[i].addEventListener('load', reset);
              images[i].removeEventListener('error', reset);
              images[i].addEventListener('error', reset);
            }
          }
        }
      ...
      (function() {
        ...
        if (!document.getElementById('flagstone-' + instanceIndex + '-styles')) {
          ...
          const stoneAnimationStyles = '.flagstone-' + instanceIndex + '-bed__stone{transition:top ' + animationDurationSeconds + 's ' + animationTimingFunction + ',' + direction + ' ' + animationDurationSeconds + 's ' + animationTimingFunction + (flow ? ',width ' + animationDurationSeconds + 's ' + animationTimingFunction : '') + (heightAnimationDuration ? ',height ' + heightAnimationDurationSeconds + 's ' + heightAnimationTimingFunction : '') + ';}';
